...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        {
            //Print() << Particles::get_particles() << std::endl;
            MultiFab&  S_new    = get_new_data(State_Type);
...
#endif
#if (AMREX_SPACEDIM == 2)
if (level == Fibers::FiberFinestLevel())//parent->finestLevel())
        {
            //Print() << "Finest level  " << std::endl;
            //if (Fibers::get_fibers() != nullptr) {
            //    Print() << "Fibers::get_fibers() != nullptr" << std::endl;
            //}
            //Print() << Fibers::get_fibers() << std::endl;
            MultiFab&  S_new    = get_new_data(State_Type);
            // S_new.setVal(1.0, 0, 1, S_new.nGrow()); // u = 1
            // S_new.setVal(2.0, 1, 1, S_new.nGrow()); // v = 2
            // S_new.setVal(3.0, 2, 1, S_new.nGrow()); // w = 3
            MultiFab EulerForce(S_new.boxArray(), S_new.DistributionMap(), 2, S_new.nGrow());            
            Fibers::get_fibers()->InteractWithEuler(S_new, EulerForce, dt); // parent->levelSteps(0), time
        }
        //amrex::Abort("Stop here!");
#endif
#endif
        ...
        if (level == Fibers::FiberFinestLevel())//parent->finestLevel())
        {
            MultiFab&  S_new    = get_new_data(State_Type);
            MultiFab&  S_old    = get_old_data(State_Type);
            Fibers::get_fibers()->UpdateFibers(parent->levelSteps(0), time, dt);
        }
#endif
#endif
